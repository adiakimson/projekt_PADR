...
test_data<-subset(mydata_siec,select=-c(6))
train_data<-subset(mydata_siec,select=-c(1:5))
set.seed(1234)
trControl <- trainControl(method = "cv",
                          number = 10,
                          search = "grid")
rf_default <- train(Smierc~.,data=mydata_siec,method = "rf",
                    metric = "RMSE",
                    trControl = trControl)
print(rf_default)
rf_default$results
...
fit_rf <- train(Smierc~.,
                ...
                maxnodes = 15)
#prediction
prediction <-predict(fit_rf, mydata_siec)
#confusion matrix
smierc_vec<-as.numeric(unlist(mydata_siec$Smierc))
confusionMatrix(
